[![Gem Version](https://badge.fury.io/rb/dnsmadeeasy.png)](https://badge.fury.io/rb/dnsmadeeasy) [![Downloads](https://img.shields.io/gem/dt/dnsmadeeasy?style=for-the-badge)](https://rubygems.org/gems/dnsmadeeasy)
...
> [!CAUTION] 
>
> This gem not backwards compatible with the previous version. It has been updated to work with DNS zone files and folllow Terraform's pattern of `read` → `plan`  → `apply` loop.
...
This library now ships with the `dme` executable. The CLI is organized around subcommands:

* `dme account ...` for the legacy API operations that were previously exposed at the top level
* `dme zone ...` for zone-file export, formatting, planning, and applying

Run `dme --help` to see the available command groups, and `dme account --help` or `dme zone --help` for the command-specific usage.

![Usage](.dme-help.png)

The `account` subcommand keeps the existing API operations available under a single namespace:
...
The zone workflow is designed for the `read -> plan -> apply` loop. The example below uses `example.com` and a fresh set of TXT values, but follows the same steps used in a real apply:

```bash
❯ bundle exec dme zone export example.com --output=example.com.current.zone
❯ bundle exec dme zone validate example.com.current.zone
❯ bundle exec dme zone fmt example.com.current.zone > example.com.resend.zone
❯ bundle exec dme zone plan example.com.resend.zone --domain=example.com
❯ bundle exec dme zone apply example.com.resend.zone --domain=example.com --add-only --yes
```

An example `example.com.resend.zone` file looks like this:

```dns
$ORIGIN example.com.
$TTL 300

click    IN CNAME   links1.resend-dns.com.

@        IN MX      10 inbound-smtp.us-east-1.amazonaws.com.
send     IN MX      10 feedback-smtp.us-east-1.amazonses.com.

_dmarc   IN TXT     "v=DMARC1; p=none;"
resend._domainkey IN TXT     "p=MIGfMA0GCSqGSIb3DQEBAQABAAABAQDJk8rYw6r9eV7J1e5q1v0Cw7Lh8c3b4GfP8Y2WfWQ2kX0k9Q2l2NwQxR5zV3d8xT0y7G3mHcP0yT8m0j1XQ5n0a9rR8yJf6G3GvQ1X1c9tF8pQ4vZ0x8Q7mN2uS5fE8yY0Wm7Z1Q4mV2Q6gH9bN1sR3tW8xQ7bV5xY2mJ8cL1pM3wR6uT9yA0bC1dE2fG3hI4jK5lM6nO7pQ8"
